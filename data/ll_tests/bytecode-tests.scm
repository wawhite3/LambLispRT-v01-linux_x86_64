...
;;; -----------------------------------------------------------------------
;;; B244: eq? must give the SAME answer interpreted and compiled.
;;;
;;; R7RS 6.1 leaves eq? on numbers unspecified, so either answer is acceptable ALONE -- what is
;;; not acceptable is the two execution modes disagreeing about one expression (the B190/B192/B193
;;; lesson).  These tests therefore compare AST against BYTECODE rather than against a fixed
;;; answer, except where the answer is now pinned by the fix.
;;;
;;; NaN IS THE ONLY VALUE THAT CAN EXPOSE THIS, so do not "simplify" these to 1.5.  An interpreted
;;; float lookup materialises a fresh cell each time, so eq? sees two distinct cells holding equal
;;; bits; for every ordinary float the old value-compare still answered #t and the divergence was
;;; invisible.  NaN is the only value not equal to itself under ==.  Testing with 1.5 and seeing
;;; #t is consistent with BOTH "same cell" and "different cells, equal values" -- that false
;;; disproof was drawn once during the investigation and had to be reversed.
;;; -----------------------------------------------------------------------

(printf "--- B244 eq? mode agreement ---\n")

(define (b244-nan) (/ 0. 0.))

(define (b244-eq-self-ast)  (let ((x (b244-nan))) (eq? x x)))
(define (b244-eq-self-bc)   ((procedure->bytecode (lambda () (let ((x (b244-nan))) (eq? x x))))))

(check "B244 eq? NaN self: modes agree" (b244-eq-self-ast) (b244-eq-self-bc))
(check-true "B244 eq? NaN self is #t"   (b244-eq-self-ast))

;;; Ordinary floats must keep agreeing too -- they always did, but a bit-compare could have
;;; changed them, so this pins the property rather than assuming it.
(check "B244 eq? 1.5 self: modes agree"
       (let ((x 1.5)) (eq? x x))
       ((procedure->bytecode (lambda () (let ((x 1.5)) (eq? x x))))))

;;; Signed zero: bits differ, so eq? is now #f -- which is what eqv? says (B240), because
;;; (/ 1 0.0) and (/ 1 -0.0) differ.  The old == compare said #t here.
(check "B244 eq? 0.0 vs -0.0: modes agree"
       (eq? 0.0 -0.0)
       ((procedure->bytecode (lambda () (eq? 0.0 -0.0)))))
(check-false "B244 eq? 0.0 vs -0.0 is #f" (eq? 0.0 -0.0))

;;; eqv? was fixed separately in B240 and must be unaffected by the eq? change.
(check-true  "B244 eqv? NaN self still #t"     (let ((x (b244-nan))) (eqv? x x)))
(check-false "B244 eqv? 0.0 -0.0 still #f"     (eqv? 0.0 -0.0))
(check-true  "B244 eqv? 2.0 (sqrt 4.0) still #t" (eqv? 2.0 (sqrt 4.0)))
