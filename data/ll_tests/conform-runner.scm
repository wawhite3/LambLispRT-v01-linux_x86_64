...
(define (%exc->fn name)                          ;; the R7RS function a failing case exercises
  ;; NOTE: the first clause is a DEFAULT, not a measurement -- it attributes any pitfall id that is
  ;; not a string to call/cc without inspecting it.  True today (all nine remaining pitfall cases
  ;; genuinely use call/cc re-entrantly); if a non-call/cc pitfall ever starts excepting, it will be
  ;; silently mislabelled here rather than reported as unclassified.
...
#|!
  CORRECTED 2026-09-05 (P168 sec 7.1).  This list used to attribute dynamic-wind, make-parameter,
  parameterize and raise-continuable to "requires first-class continuations (call/cc)".  That was
  FALSE for all four, and the cost of the wrong record was not the misinformation -- it was that
  eighteen conformance gaps looked blocked behind a deferred, expensive C++ feature, so nobody
  looked again.  They were closable in ~50 lines of Scheme (rxrs_continuations.scm).
  What is left here is only what a continuation is genuinely needed for: RE-ENTRY.
|#
(define %exc-report
  (list (cons "call-with-current-continuation" "requires RE-ENTRANT continuations (escape-only call/cc is implemented; these invoke k after its extent has exited)")
