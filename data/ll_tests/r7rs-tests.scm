...
;; Expected exceptions in a clean run: RE-ENTRANT call/cc (1) + library system (5:
;; define-library/import/only/except/prefix/rename) = 6.
;;
;; Was 24.  P168 closed 18 of them in pure Scheme (rxrs_continuations.scm): dynamic-wind (3),
;; make-parameter/parameterize (9), raise-continuable (1) and the ESCAPE-SHAPED call/cc cases (5).
;; None of those ever needed first-class continuations -- resuming a raise-continuable never leaves
;; the continuation, and an escape is a tagged raise.  What remains is genuinely continuation-
;; dependent: invoking k AFTER its extent has exited, which escape-only cannot do at any price.
(define r7rs-exception-expected 6)
