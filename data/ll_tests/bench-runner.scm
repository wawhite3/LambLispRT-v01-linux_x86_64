    ...
    (let loop ((reps 1))
      (if (and (< (- (current-us) t0) bench-window-us)
          ...
          (let* ((dt   (- (current-us) t0))
            ...
            (display reps)
            ;;; P143 Part A -- PER-BENCHMARK VALUE-VERIFY EVIDENCE, on EVERY line.
            ;;; Until now a value appeared only on MISMATCH, so "this benchmark was checked and
            ;;; matched" could only be inferred from the ABSENCE of a failure marker.  Negative
            ;;; evidence is exactly what R11 exists to replace: a record must say what happened, not
            ;;; leave it to be deduced from what is missing.
            ;;; WHY THE OUTCOME AND NOT THE RAW VALUE.  uLisp prints `val=4` because its benchmarks
            ;;; return scalars; ours do not -- iota(500) returns a 500-element LIST and qsort(200) a
            ;;; 200-element vector.  Printing those on every line would flood the serial link the
            ;;; perf tier already needed `,quiet` to keep readable (P137 Ph5), and reproducing the
            ;;; harness's digest rules here would put the expected values in two places.  The check
            ;;; already ran against the right answer; what was missing from the record is only its
            ;;; OUTCOME, so that is what is emitted.
            (display " verified=")
            (display (if ok 1 0))
