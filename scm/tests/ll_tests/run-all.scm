...
;;; LLArduino (P176) conformance.  Runs on EVERY target: the portable tier
;;; asserts timing, bit ops and Arduino Print fidelity against whichever
;;; backend is compiled in, so a host and a device disagreeing shows up here
;;; rather than in a serial monitor months later.  Pin-driving tests are
;;; gated to the simulated backend -- a suite must never drive an output on
;;; a board it cannot see.
(load "ll_tests/llarduino-tests.scm" 0)

(run-all-tally! 'llarduino-test-pass 'llarduino-test-fail)
