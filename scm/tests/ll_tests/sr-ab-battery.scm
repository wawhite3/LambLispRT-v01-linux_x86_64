...
;;; B206: THE ESCAPE HALF.  R7RS 4.3.2 has two devices for writing a macro that writes a macro --
;;; the custom ellipsis (cases above) and the escape (<ellipsis> <template>).  The battery covered
;;; only the first, which is exactly how the two implementations came to fail on OPPOSITE halves
;;; without the A/B noticing: C++ had the escape and not the custom ellipsis, Scheme had the custom
;;; ellipsis and not the escape.  A battery that tests one half of a symmetric feature reports
;;; agreement it has not checked.
(srab "escape/macro-writes-macro"
      (lambda ()
        (eval '(let ()
                 (define-syntax sr-blb2
                   (syntax-rules ()
                     ((_ name)
                      (define-syntax name (syntax-rules () ((_ e (... ...)) (begin e (... ...))))))))
                 (sr-blb2 sr-seq2)
...
;;; The escape with a pattern variable inside it: (... (x ...)) must substitute x and keep the
;;; ellipsis literal, so this is not merely quotation.
(srab "escape/substitutes-pvar"
      (lambda ()
        (eval '(let ()
                 (define-syntax sr-esc-pv (syntax-rules () ((_ x) '(... (x ...)))))
                 (sr-esc-pv 42))
              (interaction-environment))))
