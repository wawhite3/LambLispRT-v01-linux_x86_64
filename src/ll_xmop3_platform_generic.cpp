...
#include <malloc.h>   //!< B289: mallinfo2 for Platform.sysmem-inuse
#endif
#include "ll_vm_ncg.h"
...
//! LLArduino (P176) introspection.  These exist so the layer's Arduino-fidelity guarantees are
...
//! B289: (Platform.sysmem-inuse) -> bytes currently held by the SYSTEM allocator, or -1.
//!
...
//! cell -- measured 2026-09-06, a `(make-vector 5000)` moved the free-cell count by FIVE.  So a
...
Sexpr_t Platform_mop3_gc_diag(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  //! Index 13/14: B289 -- instantaneous cell-pool occupancy, for the idle/leak test.  The list
  //! is built by CONSING, so the FIRST value cons'd ends up LAST: to append at the tail these
  //! must go BEFORE yuasa_N, and total_cells before nfree.  Read them as (13)=nfree,
  //! (14)=total_cells.  Never insert -- gc-pause-bench.scm and idle-heap-tests.scm are
  //! positional.
  mop3_gc_protect(r, { r = lamb.cons(lamb.mk_integer(lamb.mem_dbg_total_cells(), env_exec), r, env_exec); });
  mop3_gc_protect(r, { r = lamb.cons(lamb.mk_integer(lamb.mem_dbg_nfree(), env_exec), r, env_exec); });
...
static Sexpr_t Platform_mop3_get_env_vars(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  for (char **e = environ; e && *e; e++) {
    ...
    mop3_gc_protect(res, {                       // re-root accumulator across the allocations below
        //!< B251 class: `nm` is a C-STACK local and the next mk_string ALLOCATES, so a GC quantum
