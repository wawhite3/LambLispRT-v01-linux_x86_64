...
//! B256: POINT THIS AT THE ARDUINO GLOBAL.  It was declared and NEVER ASSIGNED anywhere in the
...
//! B256: IS THE STA INTERFACE UP ENOUGH TO BE ASKED FOR STATUS?
//!
...
//! Arduino 2.0.17 the same call was safe, so this is a MIGRATION REGRESSION (B256), not a latent
...
//! ran" instead of "this broke" (same class as B211/B145).
//!
...
{
  //! B256: NOT STARTED IS NOT CONNECTED.  A predicate named `isConnected` must answer the question,
...
{
  //! B256: status() reads the same NetworkInterface state isConnected() does, so it is the direct
