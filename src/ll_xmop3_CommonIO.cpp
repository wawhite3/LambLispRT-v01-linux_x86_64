...
//! noTone had a backend all along -- both the vendor core and LLArduino provide it -- so the stub
//! was silently discarding a call that would have worked.
Sexpr_t mop3_cio_noTone(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ noTone(lamb.car(sexpr)->mustbe_int32());  return OBJ_VOID; }
...
//! B259/P176: these seven were registered but returned #UNDEF on EVERY target, including the
//! ESP32 boards -- callable, documented, and silently useless.  They are pure arithmetic with no
//! hardware behind them, so there was never a platform reason for the stub.  Found by the LLArduino
//! conformance suite on its first run.
//! DIVERGENCE FROM ARDUINO, STATED: Arduino's bitSet/bitClear/bitWrite are MACROS that MUTATE their
//! first argument in place.  Scheme has no integer to mutate, so these RETURN the new value and the
//! caller rebinds.  Same result, different shape; a sketch ported verbatim must assign.
Sexpr_t mop3_cio_bit(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ return lamb.mk_integer(1L << lamb.car(sexpr)->coerce_int32(), env_exec); }
//! Clear a specific bit in an integer (stub).
Sexpr_t mop3_cio_bitClear(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ LL_int32 v = lamb.car(sexpr)->coerce_int32(), n = lamb.cadr(sexpr)->coerce_int32();  return lamb.mk_integer(v & ~(1L << n), env_exec); }
//! Read a specific bit from an integer (stub).
Sexpr_t mop3_cio_bitRead(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ LL_int32 v = lamb.car(sexpr)->coerce_int32(), n = lamb.cadr(sexpr)->coerce_int32();  return lamb.mk_integer((v >> n) & 1, env_exec); }
//! Set a specific bit in an integer (stub).
Sexpr_t mop3_cio_bitSet(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ LL_int32 v = lamb.car(sexpr)->coerce_int32(), n = lamb.cadr(sexpr)->coerce_int32();  return lamb.mk_integer(v | (1L << n), env_exec); }
//! Write a specific bit in an integer (stub).
Sexpr_t mop3_cio_bitWrite(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ LL_int32 v = lamb.car(sexpr)->coerce_int32(), n = lamb.cadr(sexpr)->coerce_int32();  Sexpr_t b = lamb.caddr(sexpr);  bool hi = (b != HASHF) && !(b->is_any_int() && b->coerce_int32() == 0);  return lamb.mk_integer(hi ? (v | (1L << n)) : (v & ~(1L << n)), env_exec); }
//! Return the high byte of a two-byte integer (stub).
Sexpr_t mop3_cio_highByte(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ return lamb.mk_integer((lamb.car(sexpr)->coerce_int32() >> 8) & 0xFF, env_exec); }
//! Return the low byte of a two-byte integer (stub).
Sexpr_t mop3_cio_lowByte(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)			{ return lamb.mk_integer(lamb.car(sexpr)->coerce_int32() & 0xFF, env_exec); }
