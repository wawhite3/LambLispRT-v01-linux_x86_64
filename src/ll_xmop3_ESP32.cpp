...
//! esp_efuse_mac_get_default() and friends MOVED OUT of esp_system.h into esp_mac.h in ESP-IDF 5.
//! esp_mac.h DOES NOT EXIST in IDF 4.4, so this cannot be an unconditional include -- hence
//! __has_include rather than a version test, which also keeps it correct if the header ever moves
//! again.  The symptom without it is a bare "'esp_efuse_mac_get_default' was not declared in this
//! scope" plus a did-you-mean pointing at THIS FILE'S OWN wrapper (mop3_esp32_efuse_mac_get_default),
//! which is a convincing wrong lead: it reads as a self-reference or a typo rather than as a header
//! that moved.
