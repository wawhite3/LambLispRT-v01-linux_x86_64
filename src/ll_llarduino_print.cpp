...
//!
//! P176.  Print and Stream are pure character I/O, and LambStdio already implements exactly that on
...
//! while a device disagrees, which is the failure P176 criterion 3 is designed to catch:
...
static size_t ll_print_ulong(Print &p, unsigned long v, int base)
{
  ...
  while (v) { unsigned long d = v % (unsigned long) base; v /= (unsigned long) base;
              ...
              //! core is the reference here.  Measured on esp32s3-n8r2-00 2026-09-04: the vendor
...
// ---------------------------------------------------------------------------
//! P176 criterion 3 is "the same suite passes against the vendor core AND against LLArduino".
...
//!
//! THEY WERE LLARDUINO-ONLY AND THAT WAS THE BUG.  Built that way, the ESP32 run returned #f from
