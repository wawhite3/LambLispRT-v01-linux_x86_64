...
#ifndef LL_FLOAT_PROFILE
#define LL_FLOAT_PROFILE 0            //!<P169 measurement build only; see ll_vm_cell.cpp.
#endif
#if LL_FLOAT_PROFILE
#define LL_FLOAT_PROF_N     5
#define LL_FLOAT_PROF_TOTAL 0         //!<every mk_float32/mk_float64 call
#define LL_FLOAT_PROF_NAN   1         //!<NaN -- the ONLY value where identity and value compare disagree (B244)
#define LL_FLOAT_PROF_SPEC  2         //!<-0.0, +inf.0, -inf.0 (NaN counted separately above)
#define LL_FLOAT_PROF_SMALL 3         //!<exactly integral and within [FLT_CACHE_MIN, FLT_CACHE_MAX]
#define LL_FLOAT_PROF_MISS  4         //!<would NOT hit any proposed cache -- the branch is pure cost for these
extern LL_int64 ll_float_prof[LL_FLOAT_PROF_N];

//! P169: classify a float the way the PROPOSED cache would, WITHOUT installing the cache.  This is
//! the whole point of the measurement -- it answers "what fraction would hit" before anyone pays
//! for a branch on the allocation path.  Range mirrors the proposal's suggested -16.0 .. +16.0.
inline void ll_float_prof_note(double v)
{
  ll_float_prof[LL_FLOAT_PROF_TOTAL]++;
  if (v != v)                                     { ll_float_prof[LL_FLOAT_PROF_NAN]++;   return; }
  if (v ==  1.0 / 0.0 || v == -1.0 / 0.0)         { ll_float_prof[LL_FLOAT_PROF_SPEC]++;  return; }
  if (v == 0.0 && __builtin_signbit(v))            { ll_float_prof[LL_FLOAT_PROF_SPEC]++;  return; }  //!< -0.0
  if (v >= -16.0 && v <= 16.0) {
    const int i = (int) v;
    if ((double) i == v)                          { ll_float_prof[LL_FLOAT_PROF_SMALL]++; return; }
  }
  ll_float_prof[LL_FLOAT_PROF_MISS]++;
}
#endif
...
#define _byte2_ptr_	(&(_contents._byte[2]))
  ...
  */
  ...
  //! B250: THIS MUST COPY.  `str()` returns a String BY VALUE, so the old one-liner
  //! `return str().c_str();` handed back a pointer into a temporary that was destroyed at the end
  //! of that very expression -- every caller then read freed memory.  The symptom is not a crash
  //! but a gensym printing as garbage or as an empty name, most often inside an error message,
  //! i.e. precisely where the name was the thing you were trying to read.
  //! GCC 8.4.0 (ESP-IDF 4.4.7) does not diagnose it at all.  GCC 15.x (Arduino 3.x / IDF 5.x)
  //! reports -Wreturn-local-addr against the CALLER, any_sym_get_chars(), so the message points one
  //! frame away from the defect -- do not go looking in the caller.
  //! Size: a gensym renders as "G" + hex(get_cdr()) (ll_vm_cell.cpp:692), at most 18 bytes on a
  //! 64-bit host, so 32 is ample; the copy is clamped anyway.
  //! NOT REENTRANT, BY CONSTRUCTION: two gensym names live at once --
  //! printf("%s %s", a->any_sym_get_chars(), b->any_sym_get_chars()) -- share this single buffer
  //! and both print whichever was evaluated last.  If a caller ever needs two simultaneously, give
  //! it a caller-supplied buffer; do NOT enlarge this one, which cannot fix that.
  Charst_t  prechecked_gensym_get_chars() {
    static Char_t buf[32];					//!< one instance program-wide (inline fn local static)
    const String  s   = str();					//!< keep the temporary alive across the copy
    Charst_t      src = (Charst_t) s.c_str();
    LL_int32      i   = 0;
    for (; src[i] && i < (LL_int32) (sizeof(buf) - 1); i++) buf[i] = src[i];
    buf[i] = '\0';
    return (Charst_t) buf;
  }
...
#define THROW_BAD_TYPE { throw mk_error("%s Bad type %s", me, dump().c_str()); }
  ...
  */
  ...
  LL_int32 any_bvec_get_length()
  {
    ...
    else if (is_any_str_atom()) Nelems = any_str_get_length();     //!< B47: BYTE length of a string
    else THROW_BAD_TYPE;

    return Nelems;
  }
  
  /*! @name Byte access to STRINGS.
    B47: strings are indexed by CHARACTER (R7RS, 87e3470), which is correct but removed the only
    way to reach the underlying UTF-8 BYTES from Scheme.  Both views are wanted -- a character
    index for text, a byte index for encoding work -- so the bytevector operations accept a string
    and expose its bytes.  `(bytevector-length "<alpha>X")` is 3 where `(string-length "<alpha>X")`
    is 2, and that is the point, not an inconsistency.
    `bytevector?` deliberately still answers #f for a string: R7RS type predicates partition the
    types, and a string is not a bytevector.  What is shared is the OPERATIONS, not the type.

    THIS IS AN EXTENSION, NOT R7RS -- SAY SO IN ANY DOCUMENTATION.  R7RS 6.9 defines the bytevector
    procedures over bytevectors; a string is a type error, and the standard bridge is
    `string->utf8` / `utf8->string`, both of which LambLisp already provides.  Verified 2026-09-04
    against two independent R7RS implementations rather than assumed:

        chibi-scheme (R7RS reference)   guile --r7rs
        (bytevector-length "abc")   ->  ERROR            ERROR
        (bytevector-u8-ref "abc" 0) ->  ERROR            ERROR
        (bytevector-length (string->utf8 "<alpha>X")) -> 3   3
        (string-length "<alpha>X")                     -> 2   2

    So portable code must use `string->utf8`, and anything written against this extension will not
    move to another Scheme.  What the extension buys, and the reason it is worth having HERE, is
    that it is a ZERO-COPY VIEW: `string->utf8` allocates a bytevector and copies the bytes, which
    on a microcontroller with a fixed cell pool is exactly the allocation you were trying to avoid
    when you asked for the bytes in the first place.
    The length is strlen -- strings here are NUL-terminated UTF-8 byte buffers -- so a string
    containing a NUL byte reports short.  That is the same limit `string-length` has always had on
    this representation, not a new one introduced here.
  */
  ByteVec_t any_bvec_get_elems()
  {
    ...
    else if (is_any_str_atom()) res = (ByteVec_t) any_str_get_chars();   //!< B47: byte view of a string
    else THROW_BAD_TYPE;
    return res;
  }
  ...
  void any_bvec_get_info(LL_int32 &Nelems, ByteVec_t &elems)
  {
    ...
    else if (is_any_str_atom()) {                    //!< B47: byte view of a string -- see the note above
      CharVec_t s;
      any_str_get_info(Nelems, s);                   //!< strlen: BYTES, not characters
      elems = (ByteVec_t) s;
    }
    else THROW_BAD_TYPE;
  }
...
public:
  ...
  */
  ...
  LL_int32 mem_dbg_nfree();		//!<B289: free cells right now (instantaneous)
  LL_int32 mem_dbg_total_cells();	//!<B289: total cells in the pool right now
  ...
  void     ncg_frame_pool_free();           //!< B142: release them; nothing did, so every instance leaked the pool.
  //!@}
  ...
  */
  ...
  //! T_FLOAT32 cell.  ALWAYS ALLOCATES -- unlike mk_int32, which returns a shared sint_cache cell
  //! for small values.  That asymmetry is B244: an embedded float read through car/cdr materializes
  //! a FRESH cell every time, so `(eq? x x)` on a NaN is #f interpreted and #t compiled.  See P169
  //! before "fixing" it with a cache -- the branch is paid by the misses too, and the measurement
  //! is what decides whether it is worth it.
  Sexpr_t mk_float32(LL_float32 r, Sexpr_t env_exec)				{
#if LL_FLOAT_PROFILE
    ll_float_prof_note((double) r);
#endif
    return tcons(Cell::T_PAIR, (Word_t) 0, (Word_t) 0, env_exec)->set(r); }
  Sexpr_t mk_float64(LL_float64 d, Sexpr_t env_exec)			{
#if LL_FLOAT_PROFILE
    ll_float_prof_note((double) d);
#endif
    return tcons(Cell::T_PAIR, (Word_t) 0, (Word_t) 0, env_exec)->set(d); }					//!<T_FLOAT64 cell.
...
private:
  ...
  //Loop-based control
  Sexpr_t sym_loop;
  Sexpr_t sym_loop_deadline_ms;	//!< B273: `loop-deadline-ms` -- the frame ceiling the idle-GC donation gates on
  Sexpr_t sym_gc_idle_strategy;	//!< B273: `gc-idle-strategy` -- which idle-GC pacing strategy to use
  Sexpr_t sym_gcis_auto;	//!< 'auto     -- deadline when a target is declared, else jitter
  Sexpr_t sym_gcis_deadline;	//!< 'deadline -- budget = loop-deadline-ms - elapsed (feeds on headroom)
  Sexpr_t sym_gcis_jitter;	//!< 'jitter   -- budget = ema - elapsed             (regulates frame length)
  Sexpr_t sym_gcis_off;		//!< 'off      -- no idle collection at all (diagnostic)
